...
Private Sub CreateSubMenus
	...
	Dim csMissCode, csZero, csHigh, csLow, csAve, csAve2, csPhoto, csCancel As CSBuilder
...
'	dHeaderID = DBaseFunctions.GetRatesHeaderID(AcctClass, AcctSubClass)
	...
	If HasSeptageFee = 1 Then
		...
		If GlobalVar.BranchID = 15 Or GlobalVar.BranchID = 43 Then
			MinSeptageCum = 10
		End If
		'//////////////////////////////////////////////////////////////////////////
...
Private Sub SaveAverageBill(iPrintStatus As Int, iReadID As Int, iAcctID As Int)
	...
	BasicAmt = DBaseFunctions.ComputeBasicAmt(strSF.Val(PresAveCum), GlobalVar.BranchID, AcctClass, AcctSubClass)
...
Private Sub btnNegativeSelect_Click
	...
	SaveNegativeReading(MissCodeID, MissCode,ReadID, AcctID)
	vibration.vibrateCancel
	pnlNegativeMsgBox.Visible = False
...
Private Sub PrintBillData(iReadID As Int)
	...
	Dim FULLCUT As String = Chr(29) & "V" & Chr(0)
'	DateTime.DateFormat = "MM/dd/yyyy"
	' Hiyas
...
'	////////////////////////////////////////////////////////////////////////////////////////
		...
		If iHasSeptage = 0 Then 'Footer No Septage Fee
...
'			If (GlobalVar.BranchID = 15 Or GlobalVar.BranchID = 43 Or GlobalVar.BranchID = 55) And AcctClass = "RES" Then
			If GlobalVar.BranchID = 48 And AcctClass = "RES" Then 'Telabastagan only
				PrintBuffer = PrintBuffer & Chr(27) & Chr(97) & Chr(48) _
					...
					& Chr(27) & "!" & Chr(1) & "be an additional charge of " & Chr(27) & "!" & Chr(8) & "P2.99/CuM" & Chr(27) & "!" & Chr(1) & " as" & Chr(10) _
...
'///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		Else 'With Septage Fee
			...
			If ((GTotalDueAmt + SeptageFee) - AdvPayment) < 0 Then
				GTotalDue = 0
			Else
				GTotalDue = (GTotalDueAmt + SeptageFee) - AdvPayment
			End If
			
			If AdvPayment > TotalDueAmtAfterSC + SeptageFee Then
				GTotalAfterDue = 0
			Else
				GTotalAfterDue = (TotalDueAmtAfterSC + SeptageFee)
			End If
			
			LogColor($"Septage Fee: "$ & SeptageFee,Colors.Cyan)

'			& Chr(27) & "!" & Chr(33) & "ORDINANCE NO.: 343 S. of 2014" & Chr(10) _

			If GlobalVar.SeptageProv = 1 Then
				If GlobalVar.BranchID = 5 Then
					sepBuffer = Chr(27) & "@" _
						& Chr(27) & Chr(97) & Chr(49) _
						& Chr(27) & "!" & Chr(33) & "STATEMENT OF ACCOUNT" & Chr(10) _
						& Chr(27) & "!" & Chr(1) & "For the Month of " & sBillPeriod & Chr(10) _
