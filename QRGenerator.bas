...
Sub Class_Globals
	...
	Private TempBB As BytesBuilder
...
Public Sub Create(Text As String) As B4XBitmap
	...
	Dim EncodedData As BytesBuilder
...
Private Sub Version4H (EncodedData As BytesBuilder)
	Dim ecs As List
	ecs.Initialize
	...
	For block1 = 0 To 3
		...
		ecs.Add(ec)
	Next
	Dim Interleaved As BytesBuilder
...
Private Sub Version4L (EncodedData As BytesBuilder)
...
Private Sub PolyDivide (This() As Int, Other() As Int) As Int()
	...
	Do While remainder.Length >= Other.Length And remainder(0) <> 0
		...
		Dim dscale As Int = GFMultiply(remainder(0), inverseDenominatorLeadingTerm)
		Dim term() As Int = PolyMultiplyByMonomial(Other, DegreeDifference, dscale)
		Dim iterationQuotient() As Int = GFBuildMonomial(DegreeDifference, dscale)
